...
class EngineManagement:
	...
	def __init__(self):
		...
		self.x_power = 0
		self.y_power = 0
		self.x_max_change_rate = REFRESH_RATE_HZ * 1
		self.y_max_change_rate = REFRESH_RATE_HZ * 1
	...
	def loop(self, x_demand: float, y_demand: float) -> None:
		...
		# self.calibration.front_left.control(y_power)
		# self.calibration.front_right.control(y_power)
		# self.calibration.steering.control(x_power)
